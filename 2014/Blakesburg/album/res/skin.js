...
(function($) {
	...
	// addModal :: adding modal windows to any layer (typically 'body')
	...
	$.fn.addModal = function( content, buttons, settings  ) {
		...
		// Ensure that rendering has finished, so we get the proper height
			...
			var nextImg = function() {
				if ( curr < images.length - 1 ) {
					...
					if ( settings.afterLast === 'startover' || to && settings.slideshowLoop ) {
						...
						/*$(document).keydown(function(e){
							log( e? e.keyCode : window.event.keyCode);
							return false;
						});*/
						/*
						setTimeout(function() {
